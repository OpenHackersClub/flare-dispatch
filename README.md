...
- **Runs** — all 17 are registered (`/health` lists them, sorted): `cdp-acceptance`, `ci-triage-pr`, `deploy-smoke`, `email-otp-login`, `finops-audit`, `matrix-fanout`, `offload-test`, `oxlint`, `playwright-demo`, `playwright-e2e`, `pr-review`, `product-demo`, `refresh-fixtures`, `release-notes`, `spec-drift-pr`, `vitest-shard`, `worker-deploy`. Headline shapes: `offload-test` (`pnpm test` → green/red check), `worker-deploy` (CD — build + `wrangler deploy` on default-branch push, credentials from the config store, green/red check on the deployed SHA), `matrix-fanout` / `vitest-shard` (N shards across containers → aggregated check; `vitest-shard` bakes Vitest's `--shard=i/n`), `oxlint` (install-free Rust lint gate → green/red check; the cheapest run — no `node_modules`), `cdp-acceptance` (boot app + CDP assertions), `playwright-demo` / `playwright-e2e` (Playwright specs → signed R2 tarball), `product-demo` (AI-driven walkthrough over CDP with Browser Run Session Recording + [rrweb replay](#how-it-works) link, per-chapter summary), `email-otp-login` (provision a fresh user through an OTP / magic-link login via a self-hosted disposable inbox), `finops-audit` (weekly model-written review of execution cost → draft PR with cost optimizations).
- **Agentic PR review** — the marquee capability. `pr-review` reviews a PR diff in-Worker via an `@effect/ai` `modelGateway` capability, configurable as **multi-agent** (up to seven per-domain personas) or **single-agent** (`pr-review.agents`), with **selectable backends chosen from `CONFIG_KV` without redeploy**: Workers AI catalog incl. DeepSeek reasoners (`@cf/…` or `deepseek/…`, binding-as-auth or BYOK — the `workers-ai` route), Anthropic-via-AI-Gateway (BYOK), and Bedrock-via-AI-Gateway (OIDC → STS → SigV4, **no long-lived AWS key**). A dispatch can override the model/region/role per call for model bake-offs. Diff is capped to the chosen model's context window; the review comment is a scannable summary table + per-finding headings + GitHub blob links; re-reviews auto-resolve fixed findings.
...
- **Email-OTP user provisioning** — a `mailbox` capability + a `waitForOtp` primitive let a run drive a real OTP / magic-link login (Auth0/Clerk/own app) end to end: `provisionInbox()` mints a disposable `demo-…@INBOX_DOMAIN`, an inbound Email Routing `email()` handler receives the verification mail and signals the paused run via `step.waitForEvent`, and the code/link is extracted and verified. The `email-otp-login` run is the worked example. **Opt-in**: the capability is a dying stub until `INBOX_DOMAIN` + an Email Routing catch-all rule are configured; default-secure (reject-before-parse, text-only at rest, expiring read token, burn-after-read). See [`recipes/email-otp-login`](recipes/email-otp-login/).
...
# {"status":"ok","runs":["cdp-acceptance","ci-triage-pr","deploy-smoke","matrix-fanout","offload-test","oxlint","playwright-demo","playwright-e2e","pr-review","product-demo","refresh-fixtures","spec-drift-pr"]}
...
├── runs/                    all 17 runs — pr-review, ci-triage-pr, spec-drift-pr, refresh-fixtures, offload-test, oxlint, release-notes, vitest-shard, cdp-acceptance, deploy-smoke, worker-deploy, matrix-fanout, playwright-demo, playwright-e2e, product-demo, email-otp-login, finops-audit
