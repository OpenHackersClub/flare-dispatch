...
- **Runs** — all 13 are registered (`/health` lists them, sorted): `cdp-acceptance`, `ci-triage-pr`, `deploy-smoke`, `matrix-fanout`, `offload-test`, `oxlint`, `playwright-demo`, `playwright-e2e`, `pr-review`, `product-demo`, `refresh-fixtures`, `release-notes`, `spec-drift-pr`. Headline shapes: `offload-test` (`pnpm test` → green/red check), `oxlint` (install-free Rust lint gate → green/red check; the cheapest run — no `node_modules`), `cdp-acceptance` (boot app + CDP assertions), `playwright-demo` / `playwright-e2e` (Playwright specs → signed R2 tarball), `product-demo` (AI-driven walkthrough over CDP with Browser Run Session Recording + [rrweb replay](#how-it-works) link, per-chapter summary).
...
├── runs/                    all 13 runs — pr-review, ci-triage-pr, spec-drift-pr, refresh-fixtures, offload-test, oxlint, release-notes, cdp-acceptance, deploy-smoke, matrix-fanout, playwright-demo, playwright-e2e, product-demo
