...
export function initTelemetry(): void {
  ...
  HyperDX.init({
    ...
    // Add W3C trace context to the SPA's same-origin /v1 fetches so they stitch
    // to backend spans if/when the dispatcher Worker emits OTel. Same-origin
    // requests never trigger a CORS preflight, so the extra header is safe.
    ...
    maskAllInputs: true,
    // advancedNetworkCapture is intentionally OFF: the /v1/dashboard.json feed
    // carries HMAC-tokened logsUrl/demosUrl deep links in its body, and capturing
    // response bodies would ship those tokens to HyperDX. We still get a span per
    // fetch (timing + status), just not the payload.
  });
}
