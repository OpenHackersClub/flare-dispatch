// @flare-dispatch/core — the `incident/v1` contract.
//
...
//
// `signals` (title/detail) and `ciFailures[].logTail` are telemetry — anyone who
// can emit a log line or fire an alert influences them. They flow into a coding
// agent that WRITES CODE, so they are a prompt-injection vector. The schema caps
// BYTES, not TRUST: the consumer of this pack MUST keep these fields fenced as
// data (never instructions), and the agent's output is still gated by
// sandbox-verify + the writeback allowlist. See specs/08-self-healing.md § 10.1.
//
...
export const IncidentClass = Schema.Literal("ci", "application");
