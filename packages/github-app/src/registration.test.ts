...
const PUBLIC_APP = {
  ...
  owner: { login: "OpenHackersClub", type: "Organization" },
  permissions: {
    checks: "write",
    contents: "read",
    deployments: "read",
...
describe("fetchPublicAppRegistration", () => {
  it("reads permissions + events from the public App view with no auth", async () => {
    ...
    expect(reg.ownerLogin).toBe("OpenHackersClub");
    expect(appSettingsUrl(reg, "flaredispatch")).toBe(
      "https://github.com/organizations/OpenHackersClub/settings/apps/flaredispatch",
    );
  });
