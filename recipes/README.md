...
| [security-posture](security-posture/) | Weekly least-privilege / defense-in-depth audit of the BYOC deployment itself → draft PR with a posture scorecard | Schedule | `security-posture.run.ts`, `README.md` |
...
The Action-mode `*.run.ts` files mirror the shipped runs catalogued in [specs/02-runs.md](../specs/02-runs.md), reproduced in each recipe so it is self-contained. `pr-review`, `spec-drift-pr`, and `ci-triage-pr` are likewise shipped runs (in `runs/`), reproduced here. The remaining Webhook- and Schedule-mode recipes — `deploy-smoke`, `pr-review-sweep`, `nightly-e2e`, `release-notes`, `scheduled-deps`, `security-posture` — are custom runs defined by the recipe itself; those Schedule-mode runs are thin orchestration that dispatch shipped runs (`playwright-e2e`, `security-scan`) or, for `release-notes`, shell out to a bundled CLI.

`spec-drift-pr`, `ci-triage-pr`, and `security-posture` are the "detect → draft PR" recipes: a Schedule-mode run reuses the `ai-code-review` engine's configurable `workers-ai` backend (via `@flare-dispatch/review-agent`'s `completeStructured`) to detect drift / triage CI failures / audit the deployment's own security posture, then opens a **draft PR** through `github.openDraftPullRequest` (the Git Data API, from the Worker). Downstream operators customize each recipe's model and prompt entirely from `CONFIG_KV` under its own `spec-drift.*` / `ci-triage.*` / `security-posture.*` namespace. `security-posture` differs in one way: its deterministic rule checks stand alone, so a missing or failing model backend degrades it to a rules-only report rather than failing the run — a security control should never go dark.
