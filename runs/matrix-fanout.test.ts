...
describe("matrix-fanout", () => {
  ...
  it.effect(
    "mixed path — one shard exits 1, run Effect succeeds, failed === 1",
    () => {
      ...
      return Effect.gen(function* () {
        ...
        if (Exit.isSuccess(exit)) {
          ...
          expect(
            exit.value.shardResults.every((r) => r.exitCode === 1),
          ).toBe(true);
        }
      }).pipe(Effect.provide(layer));
    },
  ...
  it.effect(
    "env wiring — every shard gets SHARD_INDEX / SHARD_TOTAL injected",
    () => {
      const { layer, handles } = makeCFRuntimeTest({
        sandboxProgram: { "pnpm test": { exitCode: 0 } },
      });
