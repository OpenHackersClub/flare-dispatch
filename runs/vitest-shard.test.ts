...
describe("vitest-shard", () => {
  it.effect(
    "green path — every shard exits 0, failed === 0, passed === shards",
    () => {
  ...
  it.effect(
    "all-red — every shard exits 1, the run Effect succeeds, failed === shards",
    () => {
      ...
      return Effect.gen(function* () {
        ...
        if (Exit.isSuccess(exit)) {
          ...
          expect(exit.value.shardResults.every((r) => r.exitCode === 1)).toBe(
            true,
          );
        }
      }).pipe(Effect.provide(layer));
  ...
  it.effect(
    "timeout — a shard raises ExecTimeout, the run re-fails with the same tag",
    () => {
      const { layer } = makeCFRuntimeTest({
  ...
  it.effect(
    "shard flag — each shard runs `<command> --shard=<i>/<N>`; install ran first",
    () => {
      const { layer, handles } = makeCFRuntimeTest({
        sandboxProgram: { [VITEST_CMD]: { exitCode: 0 } },
      });
      ...
      return Effect.gen(function* () {
        ...
        const shardCmds = commands
          .filter((c) => c.includes("--shard="))
          .sort();
