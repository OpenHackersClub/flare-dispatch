...
The smallest end-to-end slice that proves the model from [PRD § Roadmap](PRD.md#success-criteria--roadmap):
...
- **D1** — `executions` + `steps` tables per [01-architecture § Storage](01-architecture.md#storage).
...
| **App-webhook trigger surface** (`POST /v1/webhooks/github`) | V0 proves the HMAC-POST dispatch path end-to-end. The autonomous App-webhook trigger ([04-gha-integration § Pattern 2](04-gha-integration.md)) adds receiver-side gate logic, run-level trigger config, and `check_run.rerequested` re-run handling — orthogonal to "Sandbox → check-run." → V1. |
...
├── infra/
│   └── d1-schema.sql                           # executions + steps tables verbatim from 01-architecture § Storage
├── actions/
...
If steps 4, 6, 7, and 10 all pass, V0 is complete and the [PRD § Roadmap](PRD.md#success-criteria--roadmap) exit criterion is met.
