...
- **What:** `SandboxCloudflareLive.runDetached` / `waitForExit` / `waitForPort` — the V0 `Effect.die` stubs replaced with the `@cloudflare/sandbox` `startProcess` / process-polling / `exposePort` surface, so the `bootApp` primitive works. `BrowserService.newCDPSession` live against Cloudflare Browser Rendering — `newCDPSession` returns the REST `/connect` `wss://` endpoint the container's Playwright process dials directly with a `CLOUDFLARE_API_TOKEN` (resolved through `loadSecrets` from PR8). `recipes/cdp-acceptance` is promoted to `runs/cdp-acceptance.ts` and registered in both the Dispatcher registry and `RunWorkflow`. `wrangler.jsonc` adds the `browser` binding.
- **Verifiable acceptance:** `cdp-acceptance` unit-tested against `CFRuntimeTest` + a `sandboxFakeProgram` (green / red / boot-timeout paths), the same pattern as `runs/offload-test.test.ts`. Live detached + Browser Rendering Layers are typecheck + `wrangler deploy --dry-run` verified; the end-to-end acceptance suite is a `wrangler dev` smoke. This closes the [roadmap](#roadmap) V2 exit criterion.
